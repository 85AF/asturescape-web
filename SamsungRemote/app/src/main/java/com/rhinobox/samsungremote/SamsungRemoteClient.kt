...
class SamsungRemoteClient(private val onStatus: (String, Boolean) -> Unit) {
    ...
    private fun open(base: String) {
        val name = Base64.encodeToString("Universal TV Remote".toByteArray(), Base64.NO_WRAP)
        val url = buildString {
    ...
    fun launchStreaming(service: String) {
        val ids = when (service.lowercase()) {
            "netflix" -> listOf("3201907018807", "11101200001")
            "prime", "primevideo", "prime video" -> listOf("3201910019365", "3201512006785")
            "youtube" -> listOf("111299001912")
            else -> emptyList()
        }
        ...
        Thread {
            var launched = false
            for (id in ids) {
                if (postApplication(id, secure = false) || postApplication(id, secure = true)) {
                    launched = true
                    break
                }
            }
    ...
    fun launchApp(appId: String) {
        when (appId) {
            "11101200001", "3201907018807" -> launchStreaming("netflix")
            "3201512006785", "3201910019365" -> launchStreaming("prime")
            "111299001912" -> launchStreaming("youtube")
            else -> Thread {
                if (!postApplication(appId, false) && !postApplication(appId, true)) {
                    emitLaunch(appId, "DEEP_LINK")
                    emitLaunch(appId, "NATIVE_LAUNCH")
                }
            }.start()
        }
    }

    private fun postApplication(appId: String, secure: Boolean): Boolean {
        if (ip.isBlank()) return false
        val scheme = if (secure) "https" else "http"
        val port = if (secure) 8002 else 8001
        val url = "$scheme://$ip:$port/api/v2/applications/$appId"
        return try {
    ...
    private fun emitLaunch(appId: String, actionType: String) {
        val data = JSONObject()
            .put("appId", appId)
